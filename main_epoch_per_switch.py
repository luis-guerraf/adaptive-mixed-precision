...
Options_bitA = [2, 8, 'random']
Options_bitW = [2, 8, 'random']
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    if args.evaluate:
        for bitA in range(len(Options_bitA)):
            for bitW in range(len(Options_bitW)):
                configure_model(convs, bns, bitA, bitW)
		        validate(val_loader, model, criterion, 0, args, bitA, bitW)
        return
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        for bitA in range(len(Options_bitA)):
            for bitW in range(len(Options_bitW)):
                ...
                # train for one epoch
                train(train_loader, model, criterion, optimizer, epoch, args, bitA, bitW)

                # evaluate on validation set
                acc1 = validate(val_loader, model, criterion, epoch, args, bitA, bitW)
...
def train(train_loader, model, criterion, optimizer, epoch, args, bitA, bitW):
    ...
    progress = ProgressMeter(
        ...
        prefix="Epoch: [{}]\t bitA:{}\t bitW:{}\t".format(epoch, Options_bitA[bitA], Options_bitW[bitW]))
...
def validate(val_loader, model, criterion, epoch, args, bitA, bitW):
    batch_time = AverageMeter('Time', ':6.3f')
    losses = AverageMeter('Loss', ':.2e')
    top1 = AverageMeter('Acc@1', ':6.2f')
    top5 = AverageMeter('Acc@5', ':6.2f')
    progress = ProgressMeter(
        len(val_loader),
        [batch_time, losses, top1, top5],
        prefix='Test:  [{}]\t bitA:{}\t bitW:{}\t'.format(epoch, Options_bitA[bitA], Options_bitW[bitW]))
